...
for i in range(12):         #一年有12個月           
    for j in range(471):        #一個月有471筆資料, ex:[1~10, 2~11, ..., 471~480]
        ...
        for k in range(18):    #18個測項 
            if k not in [8, 9, 10]: # select feature, 8:PM10, 9:PM2.5
                continue
...
w = np.zeros(len(x[0]))   #w = weight, total 18(測項)*9(9小時當作一組features)=bias+162=163, 163*1的矩陣 
x_t = x.transpose()   
...
with open(args[2], 'r', encoding = 'big5') as csvfile:
    ...
    for row in text:
        if (i%18) == 0:
            j = j + 1
            test_x.append([])
        if (i%18) not in [8, 9, 10]:  #select features
            i = i + 1
            continue
        #print(j)
        for k in range(2, 11):
