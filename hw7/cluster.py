...
test = pd.read_csv(csv_dir).values
...
autoencoder = load_model('./best_model_95047.h5')
...
X_2d = TSNE(n_jobs=12, random_state=18, n_iter=3000).fit_transform(pred[0])
...
for i in range(1000000) :
    if labels[ img_dict[test[i][1]] ] in A:
        a = 0
    else:
        a = 1
    if labels[ img_dict[test[i][2]] ] in A:
        b = 0
    else:
        b = 1
    if a == b:
